...
namespace Intervu.API.Test.ApiTests.AdminController
{
    public class GetRoomReportsTests : BaseTest, IClassFixture<BaseApiTest<Program>>
    {
        ...
        public GetRoomReportsTests(BaseApiTest<Program> factory, ITestOutputHelper output) : base(output)
        {
        ...
        [Trait("Category", "API")]
        [Trait("Category", "Admin")]
        public async Task GetRoomReports_WithPaging_ReturnsSuccess()
        {
            // Arrange
            var token = await LoginAdminAsync();

            // Act
            var response = await _api.GetAsync("/api/v1/admin/room-reports?page=1&pageSize=5", jwtToken: token, logBody: true);

            // Assert
            await AssertHelper.AssertEqual(HttpStatusCode.OK, response.StatusCode, "Paged room reports returns 200 OK");
            var apiResponse = await _api.LogDeserializeJson<object>(response);
            await AssertHelper.AssertTrue(apiResponse.Success, "API response indicates success");
        }
        ...
        [Trait("Category", "API")]
        [Trait("Category", "Admin")]
        public async Task GetRoomReports_FilterByPendingStatus_ReturnsSuccess()
        {
            // Arrange
            var token = await LoginAdminAsync();

            // Act - Status 0 = Pending
            var response = await _api.GetAsync("/api/v1/admin/room-reports?status=0", jwtToken: token, logBody: true);

            // Assert
            await AssertHelper.AssertEqual(HttpStatusCode.OK, response.StatusCode, "Filter by Pending status returns 200 OK");
            var apiResponse = await _api.LogDeserializeJson<object>(response);
            await AssertHelper.AssertTrue(apiResponse.Success, "API response indicates success");
        }
        ...
        [Trait("Category", "API")]
        [Trait("Category", "Admin")]
        public async Task GetRoomReports_FilterByResolvedStatus_ReturnsSuccess()
        {
            // Arrange
            var token = await LoginAdminAsync();

            // Act - Status 1 = Resolved
            var response = await _api.GetAsync("/api/v1/admin/room-reports?status=1", jwtToken: token, logBody: true);

            // Assert
            await AssertHelper.AssertEqual(HttpStatusCode.OK, response.StatusCode, "Filter by Resolved status returns 200 OK");
            var apiResponse = await _api.LogDeserializeJson<object>(response);
            await AssertHelper.AssertTrue(apiResponse.Success, "API response indicates success");
        }
        ...
        [Trait("Category", "API")]
        [Trait("Category", "Admin")]
        public async Task GetRoomReports_PageAndPageSizeZero_ReturnsSuccessWithNormalizedPaging()
        {
            // Arrange
            var token = await LoginAdminAsync();
        ...
        [Trait("Category", "API")]
        [Trait("Category", "Admin")]
        public async Task GetRoomReports_HighPageNumber_ReturnsSuccess()
        {
            // Arrange
            var token = await LoginAdminAsync();

            // Act
            var response = await _api.GetAsync("/api/v1/admin/room-reports?page=9999&pageSize=10", jwtToken: token, logBody: true);
        ...
        [Trait("Category", "API")]
        [Trait("Category", "Admin")]
        public async Task GetRoomReports_ReturnsForbidden_WhenUserIsNotAdmin()
        {
            // Arrange
        ...
        [Trait("Category", "API")]
        [Trait("Category", "Admin")]
        public async Task GetRoomReports_ReturnsUnauthorized_WhenNoToken()
        {
            // Act
